...
int cnt_value(State rhs, Point point){
    State tmp;
    tmp.reset();
    for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
    ...
    for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
            /*if(tmp.board[i][j]==player) value += score[i][j]*1;
            else if(tmp.board[i][j]==3-player) value += score[i][j]*(-1);*/
            if(tmp.board[0][0]==player)
            {
                if((i==0 && j==1) || (i==1 && j==0) || (i==1 && j==1))
                {
                    if(tmp.board[i][j]==player) value += abs(score[i][j])*1;
                    else if(tmp.board[i][j]==3-player) value += abs(score[i][j])*(-1);
                }
                else
                {
                    if(tmp.board[i][j]==player) value += score[i][j]*1;
...
int minimax_AB(int dep, int alpha, int beta, int mamiplayer, State &S, Point &point_to_move){      //mamiplayer : 1=me  0=opponent
    if(dep>=6)
    {
        return cnt_value(S, point_to_move);
    }
    if(mamiplayer) //me
    {
        int maxeval = INT_MIN;
        ...
        else
        {
            ...
            for(auto p : tmpstate.next_valid_spots){
                int eval;
